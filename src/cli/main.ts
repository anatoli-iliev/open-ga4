...
export async function main(
  ...
  const flushWarnings = (): void => {
    for (const warning of warnings) streams.err(`warning: ${redactText(warning)}\n`);
    warnings.length = 0;
  };
...
function splitFilterExpression(
  raw: string,
): { field: string; op: FilterOperator; value: string } | undefined {
  ...
  for (let i = segments.length - 2; i >= 1; i -= 1) {
    const op = segments[i]!;
    if (!(FILTER_OPERATORS as readonly string[]).includes(op)) continue;
    const field = segments.slice(0, i).join(":");
    const value = segments.slice(i + 1).join(":");
    if (field === "" || value === "") continue;
    return { field, op: op as FilterOperator, value };
  }
  return undefined;
}
