...
- **Email** anatoli@evamber.com, with `openclaw-plugin-ga4` in the subject.
...
- Rows are rendered inside a fenced block introduced, in the text above it, as values
  supplied by site visitors and not to be treated as instructions. Rows are never
  interpolated into prose. In each cell `|` is escaped and newlines are collapsed to
  spaces (`src/ga4/format.ts`), so a value cannot forge a table row or start a line of
  its own.
