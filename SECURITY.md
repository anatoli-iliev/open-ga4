...
- Report rows are rendered inside a fenced block introduced, in the text above it, as
  ...
  never interpolated into prose (`src/ga4/format.ts`). `ga4_fields` and `ga4_diagnose`
  list field, property and account names in a plain table without that fence.
- In every table cell of every tool, `|` is escaped and newlines are collapsed to
