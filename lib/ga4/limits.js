import { assertDimensionsAllowed } from "../privacy/policy.js";
...
export function applyRenames(names) {
    const rewrites = [];
...
export class Ga4RequestError extends Ga4Error {
    ...
    constructor(reason, message) {
        super("INVALID_REQUEST", message, "This was caught locally, before any request reached Google. Fix the value named above and try again.");
        this.reason = reason;
        this.name = "Ga4RequestError";
    }
}
/** Reject a request Google would reject, with a message that says what to do. */
export function assertWithinLimits(shape) {
    if (shape.dimensions.length === 0 && shape.metrics.length === 0) {
        throw new Ga4RequestError("EMPTY_REQUEST", "A report needs at least one metric. Try the 'overview' preset for headline numbers.");
    }
    if (shape.dimensions.length > LIMITS.MAX_DIMENSIONS) {
        throw new Ga4RequestError("TOO_MANY_DIMENSIONS", `Google Analytics allows ${LIMITS.MAX_DIMENSIONS} dimensions per report; this asks for ` +
            `${shape.dimensions.length} (${shape.dimensions.join(", ")}). Split it into separate reports.`);
    }
    if (shape.metrics.length > LIMITS.MAX_METRICS) {
        throw new Ga4RequestError("TOO_MANY_METRICS", `Google Analytics allows ${LIMITS.MAX_METRICS} metrics per report; this asks for ` +
            `${shape.metrics.length} (${shape.metrics.join(", ")}). Split it into separate reports.`);
    }
    if ((shape.dateRangeCount ?? 1) > LIMITS.MAX_DATE_RANGES) {
        throw new Ga4RequestError("TOO_MANY_DATE_RANGES", `Google Analytics allows ${LIMITS.MAX_DATE_RANGES} date ranges per report; this asks for ` +
    ...
    if (shape.limit !== undefined) {
        ...
        if (shape.limit > LIMITS.MAX_ROWS) {
            throw new Ga4RequestError("LIMIT_TOO_LARGE", `This skill returns at most ${LIMITS.MAX_ROWS} rows per report, to keep results readable ` +
 ...
 */
export const REALTIME_DIMENSIONS = new Set([
    "appVersion",
    "audienceId",
    "audienceName",
...
export const REALTIME_METRICS = new Set([
    "activeUsers",
    "eventCount",
    "keyEvents",
    "screenPageViews",
 ...
 */
export function assertRealtimeFields(dimensions, metrics, policy, propertyIdentifying = new Set()) {
    assertDimensionsAllowed(dimensions, policy, propertyIdentifying);
    const badDimensions = dimensions.filter((name) => !REALTIME_DIMENSIONS.has(name) && !REALTIME_CUSTOM_DIMENSION.test(name));
    if (badDimensions.length > 0) {
        throw new Ga4RequestError("REALTIME_DIMENSION_UNAVAILABLE", `Realtime reporting does not have ${badDimensions.join(", ")}. It has only: ` +
    ...
    if (badMetrics.length > 0) {
        throw new Ga4RequestError("REALTIME_METRIC_UNAVAILABLE", `Realtime reporting does not have ${badMetrics.join(", ")}. It has only: ` +
