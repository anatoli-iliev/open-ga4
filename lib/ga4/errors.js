 ...
 */
function googleHelpLink(url, fallback) {
    if (!url) {
        return fallback;
    }
    let parsed;
...
export function diagnose(error, context = {}) {
    ...
    if (error.serverDate) {
        ...
        if (Math.abs(skewMs) > SKEW_TOLERANCE_MS) {
            ...
            return new Ga4Error("CLOCK_SKEW", `This machine's clock is about ${seconds} seconds ${direction} Google's, which invalidates ` +
                `the signed token used to authenticate. Google reports ${error.serverDate.toISOString()}.`, "Turn on automatic network time, then try again. macOS and Windows: Date & Time > " +
    ...
    if (error.status === 403 && info?.reason === "SERVICE_DISABLED") {
        ...
        const link = googleHelpLink(help?.links?.[0]?.url, `https://console.cloud.google.com/apis/api/${service}/overview`);
