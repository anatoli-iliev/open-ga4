 ...
 */
export function classifyDimension(name, userScopedCustom = new Set()) {
...
export function assertDimensionsAllowed(dimensions, policy, userScopedCustom = new Set()) {
    if (policy.allowUserIdentifyingDimensions) {
        return;
    }
    const blocked = dimensions.filter((name) => classifyDimension(name, userScopedCustom) === "user-identifying");
    if (blocked.length === 0) {
        return;
    }
    throw new PolicyError(`${blocked.join(", ")} ${blocked.length === 1 ? "identifies" : "identify"} individual people, ` +
        ...
        fix: "This skill refused the question on this machine; Google was never asked. Ask for the " +
