...
export function applicationDefaultCredentialsPath(home, platform = process.platform) {
    if (platform === "win32") {
 ...
 */
function looksLikeAMisplacedKeyRatherThanAPath(value) {
 ...
 */
export function parseCredentialFile(contents, source) {
    let parsed;
    try {
    ...
    if (type === "external_account" || type === "impersonated_service_account") {
        throw new Error(`credential type "${type}" is not supported; use a service-account key or gcloud application-default credentials`);
    }
    const clientEmail = stringField(parsed, "client_email");
    const privateKey = stringField(parsed, "private_key");
    if (!clientEmail || !privateKey) {
        throw new Error("service-account file is missing client_email or private_key; download the key file again from Google Cloud");
    }
    return {
...
export async function resolveCredentials(options = {}) {
    ...
    const read = options.readFileImpl ?? ((filePath) => readFile(filePath, "utf8"));
    const probes = [];
    const candidates = [
    ...
    if (inline !== undefined && inline !== "") {
        ...
        if (looksLikeAMisplacedKeyRatherThanAPath(inline)) {
            ...
            probes.push({
                ...
                detail: "the value is not JSON and does not look like a file path either; check that a key " +
