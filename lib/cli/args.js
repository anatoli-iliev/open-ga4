...
export const COMMANDS = ["doctor", "report", "compare", "live", "query", "fields", "properties"];
/**
...
export function parseArgs(argv) {
    ...
    if (!COMMANDS.includes(command)) {
        throw new UsageError(`unknown command ${JSON.stringify(command)}. Valid commands are: ${COMMANDS.join(", ")}.`);
    }
    const positional = [];
    const flags = {};
    ...
    for (let i = 1; i < argv.length; i += 1) {
        ...
        if (FORBIDDEN_FLAGS.includes(name)) {
            throw new UsageError(`--${name} is not a flag. It changes a privacy default, so it is set by a person ` +
                `through an environment variable, not by an agent on a command line. ` +
                `See the Privacy section of SKILL.md.`);
        }
        if (!allowed.includes(name)) {
            throw new UsageError(`unknown option --${name} for ${command}. Valid options are: ` +
